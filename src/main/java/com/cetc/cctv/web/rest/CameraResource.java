...
public class CameraResource {
    ...
    private final CameraSearchRepository cameraSearchRepository;

    public CameraResource(CameraRepository cameraRepository, CameraSearchRepository cameraSearchRepository) {
        this.cameraRepository = cameraRepository;
        this.cameraSearchRepository = cameraSearchRepository;
    }
    ...
    public ResponseEntity<Camera> createCamera(@RequestBody Camera camera) throws URISyntaxException {
        ...
        cameraSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deleteCamera(@PathVariable Long id) {
        ...
        cameraSearchRepository.delete(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
    }

    /**
     * SEARCH  /_search/cameras?query=:query : search for the camera corresponding
     * to the query.
     *
     * @param query the query of the camera search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/cameras")
    @Timed
    public ResponseEntity<List<Camera>> searchCameras(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Cameras for query {}", query);
        Page<Camera> page = cameraSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/cameras");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
